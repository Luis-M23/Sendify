...
export const UsuarioService = {
  async getAll(): Promise<UserRow[]> {
    ...
    if (error)
      throw new Error(
        supabaseErrorMap[error.code] || "Error al obtener usuarios"
      );
  ...
  async getById(id: string): Promise<UserRow | null> {
    ...
    if (error)
      throw new Error(
        supabaseErrorMap[error.code] || "Error al obtener usuario"
      );
  ...
  async updateRole(id: string, role: UserRow["role"]): Promise<UserRow> {
    ...
    if (error)
      throw new Error(
        supabaseErrorMap[error.code] || "Error al actualizar rol"
      );
  ...
  async setActive(id: string, activo: boolean): Promise<UserRow> {
    ...
    if (error)
      throw new Error(
        supabaseErrorMap[error.code] || "Error al actualizar estado"
      );
